...
public interface TaskStatusRepository extends JpaRepository<TaskFinishedModel, Long> {
    ...
    @Query(nativeQuery = true, value = " SELECT c.task_id,c.title,c.content,c.create_time,d.scale_name FROM" +
            " management_task c LEFT JOIN management_scale d ON c.scale_id=d.scale_id WHERE c.task_id in" +
            "(SELECT a.task_id FROM management_task a WHERE a.university_id=?1 AND a.grade=?2 AND " +
            "a.task_id NOT in(SELECT b.task_id FROM management_task_finished b WHERE b.user_id=?3 ))")
    List<Object> getTask2(Integer university_id,String grade,Long userId);
}
