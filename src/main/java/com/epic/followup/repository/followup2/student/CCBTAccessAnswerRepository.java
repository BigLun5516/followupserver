...
public interface CCBTAccessAnswerRepository extends JpaRepository<CCBTAccessAnswerModel, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT `depress_answer`, depress_score,createtime FROM `aidoctor_ccbtaccess_answer`  " +
            "where depress_answer is not null and  userid=?1  ORDER BY createtime desc limit 5")
    List<Object> getLastestHistory1(Long userid);

    @Query(nativeQuery = true, value = "SELECT `anxious_answer`, anxious_score,createtime FROM `aidoctor_ccbtaccess_answer`  " +
            "where anxious_answer is not null and  userid=?1  ORDER BY createtime desc limit 5")
    List<Object> getLastestHistory2(Long userid);

    @Query(nativeQuery = true, value = "SELECT `sleep_answer`, sleep_score,createtime FROM `aidoctor_ccbtaccess_answer`  " +
            "where sleep_answer is not null and  userid=?1  ORDER BY createtime desc limit 5")
    List<Object> getLastestHistory3(Long userid);

    @Query(nativeQuery = true, value = "SELECT `cognition_answer`,createtime FROM `aidoctor_ccbtaccess_answer`  " +
            "where cognition_answer is not null and  userid=?1  ORDER BY createtime desc limit 5")
    List<Object> getLastestHistory4(Long userid);

    @Query(nativeQuery = true, value = "SELECT `relationship_answer`,createtime FROM `aidoctor_ccbtaccess_answer`  " +
            "where relationship_answer is not null and  userid=?1  ORDER BY createtime desc limit 5")
    List<Object> getLastestHistory5(Long userid);
